...
import { useHasPermission, useVisibleNavTools } from "@/hooks/use-nav-permissions";
...
export function AppTopNav() {
    ...
    const hasAgentPermission = useHasPermission("agent");
    const hideHeader = /^\/canvas\/[^/]+/.test(pathname);
    const slug = pathname.split("/").filter(Boolean)[0];
    const activeToolSlug = navigationTools.some((tool) => tool.slug === slug) ? (slug as NavigationToolSlug) : undefined;

    useEffect(() => {
        if (!hasAgentPermission || autoConnectRef.current || agentEnabled || agentConnected || !agentToken.trim()) return;
        autoConnectRef.current = true;
        connectAgent({ silent: true });
    }, [agentConnected, agentEnabled, agentToken, connectAgent, hasAgentPermission]);
                    ...
                    <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
                        <div className="flex min-w-0 items-center">
                            <Link to="/" className="flex h-full shrink-0 items-center gap-2 text-sm font-semibold leading-none tracking-tight text-stone-950 transition hover:text-stone-600 dark:text-stone-100 dark:hover:text-stone-300">
                                <img src="/logo.svg" alt="HowCanvas" className="size-7 shrink-0" />
                                <span className="text-base font-medium">HowCanvas</span>
                            </Link>
