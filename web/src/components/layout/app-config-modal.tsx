                ...
                items={[
                    {
                        ...
                        children: (
                            <div className="flex flex-col items-center gap-3 py-10 text-center">
                                <div className="text-sm font-medium">渠道管理已统一到管理后台</div>
                                <p className="max-w-sm text-xs text-stone-500">AI 渠道（Base URL / API Key / 模型）现由服务器统一保存，密钥不出服务器。请前往「管理后台 → 渠道与模型」添加与编辑渠道、设置默认模型。</p>
                                <Link to="/admin" onClick={() => useConfigStore.getState().setConfigDialogOpen(false)}>
                                    <Button type="primary">前往管理后台</Button>
                                </Link>
                            </div>
                        ),
                    },
...
export function AppConfigModal() {
    ...
    // 配置为管理员专属：普通用户触发（例如渠道未就绪时点「生成」）时不弹配置框，改为友好提示
    useEffect(() => {
        if (isConfigOpen && !isAdmin) {
            setConfigDialogOpen(false);
            message.warning("AI 渠道未配置或不可用，请联系管理员在服务器配置");
        }
    }, [isConfigOpen, isAdmin, message, setConfigDialogOpen]);
