...
export function CanvasNodeHoverToolbar({
    ...
    const canRecoverExistingVideo = isVideo && (Boolean(node.metadata?.videoTaskId) || /任务查询失败|取回结果失败/.test(node.metadata?.errorDetails || ""));
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canRetry ? [{ id: "retry", title: canRecoverExistingVideo ? "查询已有任务结果（不会重新扣费）" : "重新生成", label: canRecoverExistingVideo ? "取回结果" : "重试", icon: <RefreshCw className="size-4" />, onClick: () => onRetry(node) }] : []),
    ...
    const imageToolbarTools = hasImage
        ? [...toolbarTools, { id: "more", title: "配置快捷工具", label: "更多", icon: <Ellipsis className="size-4" />, active: imageToolSettingsOpen, onClick: openImageToolSettings }]
        : [];
    const imageToolbarSplitAt = Math.ceil(imageToolbarTools.length / 2);
    const imageToolbarRows = hasImage ? [imageToolbarTools.slice(0, imageToolbarSplitAt), imageToolbarTools.slice(imageToolbarSplitAt)] : [];
    ...
    return (
        <>
            <div
                className={`absolute z-[70] -translate-x-1/2 -translate-y-full overflow-visible rounded-[18px] border border-black/10 bg-white text-[15px] text-[#242529] shadow-[0_8px_28px_rgba(15,23,42,.12)] ${hasImage ? "flex flex-col" : "flex h-12 items-center"}`}
                style={{ left, top }}
            ...
            >
                {hasImage ? (
                    imageToolbarRows.map((row, index) => (
                        <div key={`toolbar-row-${index}`} className="flex h-12 items-center justify-center">
                            {row.map((tool) => (
                    ...
                    toolbarTools.map((tool) => <ToolbarAction key={tool.id} {...tool} showLabel={showImageToolLabels} />)
                )}
            </div>
            {hasImage ? (
