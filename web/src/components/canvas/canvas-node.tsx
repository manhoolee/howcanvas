...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const savedPrompt = data.metadata?.prompt?.trim() || "";
    const hideGeneratedPromptTitle = Boolean(data.metadata?.content && savedPrompt);
    ...
    return (
        ...
        >
            {!hideGeneratedPromptTitle && (isSelected || hovered || isEditingTitle) && (
            ...
            {showPanel && !isGroup && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-[840px] max-w-[calc(100vw-32px)] -translate-x-1/2 pt-4">{renderPanel(data)}</div> : null}
        </div>
    );
});
...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    const canRecoverExistingVideo = node.type === CanvasNodeType.Video && (Boolean(node.metadata?.videoTaskId) || /任务查询失败|取回结果失败/.test(node.metadata?.errorDetails || ""));
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            ...
            >
                ...
                {canRecoverExistingVideo ? "取回结果" : "重试"}
            </button>
        </div>
    );
}
