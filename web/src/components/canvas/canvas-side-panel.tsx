...
import { BookOpen, Check, ChevronRight, Download, Eye, FileText, Globe2, Image as ImageIcon, ListChecks, Music2, Plus, RefreshCw, Search, Settings2, Square, Trash2, Type, Video } from "lucide-react";
...
type PanelTab = "canvas" | "assets" | "public" | "prompts";

type Props = {
    nodes: CanvasNodeData[];
    selectedNodeIds: Set<string>;
    onFocusNode: (nodeId: string) => void;
    onRefreshNodeStatus: (nodeId: string) => Promise<void>;
...
export function CanvasSidePanel({ nodes, selectedNodeIds, onFocusNode, onRefreshNodeStatus, onInsertAsset }: Props) {
        ...
        >
            ...
            >
                <div className="flex items-center gap-5 px-4 pt-3.5">
                    ...
                    <TabButton label="公共资产" active={tab === "public"} theme={theme} onClick={() => setTab("public")} />
                    <TabButton label="提示词库" active={tab === "prompts"} theme={theme} onClick={() => setTab("prompts")} />
                </div>
                <div className="mt-2 min-h-0 flex-1 overflow-hidden">
                    {tab === "canvas" ? (
                        <CanvasNodesTab nodes={nodes} selectedNodeIds={selectedNodeIds} onFocusNode={onFocusNode} onRefreshNodeStatus={onRefreshNodeStatus} theme={theme} />
                    ) : tab === "assets" ? (
                        <CanvasAssetsTab onInsert={onInsertAsset} theme={theme} />
                    ) : tab === "public" ? (
                        <CanvasPublicAssetsTab onInsert={onInsertAsset} theme={theme} />
                    ) : (
...
function nodePreviewText(node: CanvasNodeData) {
    ...
    const typeName = getNodeDefinition(node.type)?.title || node.type;
    const taskStatus = node.metadata?.taskStatus;
    return taskStatus ? `${typeName} · ${TASK_STATUS_LABEL[taskStatus] || taskStatus}` : typeName;
}
...
function CanvasNodesTab({
    ...
    const [refreshingNodeIds, setRefreshingNodeIds] = useState<Set<string>>(new Set());
    ...
    const handleRefreshStatus = async (nodeId: string) => {
        setRefreshingNodeIds((current) => new Set(current).add(nodeId));
        try {
            await onRefreshNodeStatus(nodeId);
        } catch {
            // 错误文案由画布页统一展示，这里只负责收起加载态。
        } finally {
            setRefreshingNodeIds((current) => {
                ...
                {filtered.length ? (
                    <div className="space-y-1.5">
                        {filtered.map((node) => {
                            ...
                            const taskId = node.metadata?.serverTaskId || node.metadata?.videoTaskId;
                            const refreshing = refreshingNodeIds.has(node.id);
                            return (
                                <div
                                    key={node.id}
                                    className={cn("group flex w-full items-center rounded-lg pr-1 transition", active ? "" : "hover:bg-black/5 dark:hover:bg-white/5")}
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        disabled={!taskId || refreshing || selectMode}
                                        onClick={() => void handleRefreshStatus(node.id)}
                                        className="ml-1 grid size-7 shrink-0 place-items-center rounded-md opacity-45 transition hover:bg-black/5 hover:opacity-100 disabled:cursor-not-allowed disabled:opacity-15 dark:hover:bg-white/10"
                                        title={taskId ? `刷新任务状态·${taskId}` : "该元素没有可查询的任务 ID"}
                                        aria-label={taskId ? `刷新${node.title || "元素"}的任务状态` : "无可查询的任务 ID"}
                                    >
...
// ---------------------------------------------------------------------------
// 公共资产 Tab —— 所有用户共享的资产库,点击插入画布
// ---------------------------------------------------------------------------

const PUBLIC_ASSET_GROUPS: { kind: PublicAsset["kind"]; label: string; icon: typeof Square }[] = [
    { kind: "image", label: "图片", icon: ImageIcon },
    { kind: "video", label: "视频", icon: Video },
    { kind: "text", label: "文本", icon: FileText },
];

const CanvasPublicAssetsTab = memo(function CanvasPublicAssetsTab({ onInsert, theme }: { onInsert: (payload: InsertAssetPayload) => void; theme: CanvasTheme }) {
    const { message } = App.useApp();
    const [keyword, setKeyword] = useState("");
    const [collapsed, setCollapsed] = useState<Record<string, boolean>>({});
    const { data, isLoading, refetch, isFetching } = useQuery({ queryKey: ["canvas-public-assets"], queryFn: () => backend.publicAssets() });
    const assets = useMemo(() => data?.assets ?? [], [data]);

    const filtered = useMemo(() => {
        const query = keyword.trim().toLowerCase();
        return assets.filter((asset) => !query || [asset.title, asset.uploadedByName, ...(asset.tags || [])].join(" ").toLowerCase().includes(query));
    }, [assets, keyword]);

    const groups = useMemo(() => PUBLIC_ASSET_GROUPS.map((group) => ({ ...group, items: filtered.filter((asset) => asset.kind === group.kind) })).filter((group) => group.items.length > 0), [filtered]);

    const insert = async (asset: PublicAsset) => {
        try {
            if (asset.kind === "text") {
                onInsert({ kind: "text", content: await fetchPublicAssetText(publicAssetFileUrl(asset)), title: asset.title });
            } else if (asset.kind === "video") {
                onInsert({ kind: "video", url: publicAssetFileUrl(asset), title: asset.title });
            } else {
                onInsert({ kind: "image", dataUrl: publicAssetFileUrl(asset), title: asset.title });
            }
            message.success(`已插入「${asset.title}」`);
        } catch {
            message.error("读取公共资产失败");
        }
    };
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="flex items-center gap-2 px-4 pb-2">
                <Input size="small" allowClear prefix={<Search className="size-3.5 text-stone-400" />} placeholder="搜索公共资产" value={keyword} onChange={(e) => setKeyword(e.target.value)} />
                <button type="button" className="inline-flex size-6 shrink-0 items-center justify-center rounded-md transition-opacity hover:opacity-100" style={{ color: theme.node.text, opacity: 0.6 }} onClick={() => void refetch()} aria-label="刷新" title="刷新">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
                {isLoading ? (
                    ...
                    groups.map((group) => {
                        ...
                        return (
                            <div key={group.kind} className="mb-3">
                                <button type="button" className="flex w-full items-center gap-1.5 py-1 text-xs font-medium" style={{ color: theme.node.text, opacity: 0.65 }} onClick={() => setCollapsed((prev) => ({ ...prev, [group.kind]: !prev[group.kind] }))}>
                                    <ChevronRight className={cn("size-3.5 transition-transform", !isCollapsed && "rotate-90")} />
                                ...
                                {!isCollapsed
                                    ? group.items.map((asset) => (
                                          <div key={asset.id} className="group mt-1.5 flex items-center gap-2 rounded-lg border p-2" style={{ borderColor: theme.node.stroke, background: theme.node.fill }}>
                                              {asset.kind === "image" ? (
                                                  <img src={publicAssetFileUrl(asset)} alt={asset.title} className="size-10 shrink-0 rounded-md object-cover" loading="lazy" />
                                              ) : asset.kind === "video" ? (
                                                  <video src={publicAssetFileUrl(asset)} className="size-10 shrink-0 rounded-md bg-black object-cover" muted preload="metadata" />
                                              ) : (
                                                  <span className="flex size-10 shrink-0 items-center justify-center rounded-md" style={{ background: theme.toolbar.panel }}>
                                              ...
                                              <div className="min-w-0 flex-1">
                                                  ...
                                                  <div className="truncate text-[11px] opacity-50">{asset.uploadedByName}</div>
                                              </div>
                                              <button
                                                  type="button"
                                                  className="inline-flex size-6 shrink-0 items-center justify-center rounded-md opacity-0 transition-opacity group-hover:opacity-100"
                                                  style={{ color: theme.node.text }}
                    ...
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span className="inline-flex items-center gap-1 text-xs"><Globe2 className="size-3.5" />暂无公共资产</span>} className="pt-16" />
                )}
            </div>
        </div>
    );
