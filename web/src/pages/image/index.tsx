...
import { acknowledgeImageTaskAfterRender, type ServerImageTaskPhase } from "@/services/api/image-task";
import { deleteStoredImages, resolveImageUrl, setImageBlob, storeGeneratedImage, uploadImage } from "@/services/image-storage";
...
type GenerationResult = {
    ...
    error?: string;
    taskPhase?: ServerImageTaskPhase;
};
...
export default function ImagePage() {
    ...
    const logStore = currentUserId ? getWorkbenchLogStore("image", currentUserId) : null;
    const processedCommandRef = useRef(0);
    const agentTaskIdRef = useRef<string | undefined>(undefined);
    const activeUserIdRef = useRef<string | null>(null);
    const logSyncQueueRef = useRef(Promise.resolve());
    ...
    useEffect(() => {
        ...
        // 每次进入工作台或切换账号，都先从服务器确认当前账号的记录，
        // 再用服务器结果校正本地缓存并载入页面。
        void refreshLogs(currentUserId, getWorkbenchLogStore("image", currentUserId));
    ...
    const generate = async () => {
        ...
        const ownerIdAtStart = currentUserId;
        const authEpochAtStart = getAuthEpoch();
        const isCurrentGeneration = () => Boolean(ownerIdAtStart) && ownerIdAtStart === activeUserIdRef.current && authEpochAtStart === getAuthEpoch();
        if (!ownerIdAtStart) {
            message.error("请先登录");
            return;
        }
        ...
        setPreviewLog(null);
        setResults(Array.from({ length: generationCount }, () => ({ id: nanoid(), status: "pending", taskPhase: "queued" })));
        ...
        try {
            ...
            if (isCurrentGeneration()) message.error(error instanceof Error ? error.message : "保存生成结果失败");
        } finally {
    ...
    const downloadImage = async (image: GeneratedImage, index: number) => {
        let url = image.storageKey ? await resolveImageUrl(image.storageKey, "") : image.dataUrl;
        if (!url && image.storageKey) {
            const blob = await backend.downloadWorkbenchFile("image", image.storageKey);
            if (blob) url = await setImageBlob(image.storageKey, blob);
        }
        if (!url) {
            message.error("图片尚未缓存完成，请稍后再试");
            return;
        }
    ...
    const deleteSelectedLogs = () => {
        if (!currentUserId || !logStore) return;
        const ownerId = currentUserId;
        const store = logStore;
        const imageKeys = logs.filter((log) => selectedLogIds.includes(log.id)).flatMap((log) => log.images.map((image) => image.storageKey).filter((key): key is string => Boolean(key)));
        void Promise.all([deleteStoredImages(imageKeys), ...selectedLogIds.map((id) => store.removeItem(id))]).then(() => refreshLogs(ownerId, store));
    ...
    const saveLog = async (log: GenerationLog) => {
        ...
        const ownerId = currentUserId;
        const store = logStore;
        const save = logSyncQueueRef.current.then(async () => {
            await store.setItem(log.id, { ...serializeLog(log), updatedAt: Date.now() });
            try {
                await syncWorkbenchLogs("image", ownerId, store, true);
                // 成图已写入本地缓存后，等待本轮媒体资源完成上传，再做一次列表对齐。
                await syncWorkbenchMedia("image", ownerId, store);
                await syncWorkbenchLogs("image", ownerId, store, true);
                await syncWorkbenchMedia("image", ownerId, store);
            } catch (error) {
                console.warn("[image] 工作台云同步失败，已保留本地记录", error);
            }
            if (activeUserIdRef.current === ownerId) setLogs(await readStoredLogs(store));
        });
        logSyncQueueRef.current = save.catch(() => undefined);
        await save;
    };

    const refreshLogs = async (ownerId: string, store: WorkbenchLogStore) => {
        try {
            await syncWorkbenchLogs("image", ownerId, store);
        } catch (error) {
            console.warn("[image] 工作台云同步失败", error);
        }
        // 防止切换账号期间，旧账号的异步请求完成后覆盖新账号列表。
        if (ownerId !== activeUserIdRef.current) return;
        setLogs(await readStoredLogs(store));
        // 列表先显示，媒体缓存完成后再刷新一次 URL，确保图片可见、可下载。
        try {
            await syncWorkbenchMedia("image", ownerId, store);
            if (ownerId === activeUserIdRef.current) setLogs(await readStoredLogs(store));
        } catch (error) {
            console.warn("[image] 工作台图片缓存同步失败", error);
        }
    };
    ...
    const runGenerationSlot = async (index: number, snapshot: { text: string; config: AiConfig; references: ReferenceImage[] }, isCurrentGeneration: () => boolean, onTaskSubmitted?: (taskId: string) => void, clientContext?: Record<string, string | number | boolean>) => {
        ...
        try {
            ...
            if (isCurrentGeneration()) setResults((value) => updateResultAt(value, index, { status: "failed", error: error instanceof Error ? error.message : "生成失败" }));
            throw error;
        }
    };
    ...
    useEffect(() => {
        ...
        const pending = logs.filter((log) => log.status === "生成中" && log.taskIds?.some(Boolean));
        pending.forEach((log) => {
            if (resumedLogIdsRef.current.has(log.id)) return;
            resumedLogIdsRef.current.add(log.id);
            const ownerIdAtStart = currentUserId;
            const authEpochAtStart = getAuthEpoch();
            const isCurrentGeneration = () => ownerIdAtStart === activeUserIdRef.current && authEpochAtStart === getAuthEpoch();
            const taskIds = (log.taskIds || []).filter(Boolean);
            const resumeConfig: AiConfig = {
                ...effectiveConfig,
                ...log.config,
                model: log.model || log.config.model,
    ...
    const retryResult = async (index: number) => {
        ...
        const authEpochAtStart = getAuthEpoch();
        const isCurrentGeneration = () => Boolean(ownerIdAtStart) && ownerIdAtStart === activeUserIdRef.current && authEpochAtStart === getAuthEpoch();
        if (!ownerIdAtStart) return;
        setPreviewLog(null);
        setResults((value) => updateResultAt(value, index, { status: "pending", taskPhase: "queued", error: undefined, image: undefined }));
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            <main className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-y-auto p-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:overflow-hidden xl:grid-cols-[320px_minmax(0,1fr)]">
                ...
                <section className="grid gap-3 lg:min-h-0 lg:overflow-hidden xl:grid-cols-[520px_minmax(0,1fr)] 2xl:grid-cols-[560px_minmax(0,1fr)]">
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-3">
                                {results.map((result, index) =>
                                    result.status === "success" && result.image ? (
                                        ...
                                        <PendingImageCard key={result.id} phase={result.taskPhase} />
                                    ),
                                )}
                            </div>
                        ) : (
                            <div className="flex min-h-[260px] flex-col items-center justify-center rounded-lg border border-dashed border-stone-300 text-center dark:border-stone-700 lg:min-h-[420px]">
...
const IMAGE_PHASE_LABELS: Partial<Record<ServerImageTaskPhase, string>> = {
    queued: "排队中",
    generating: "生成中",
    "upstream-complete": "服务器已确认完成",
    retrieving: "服务器取回中",
    persisted: "前端加载中",
};

const IMAGE_PHASE_PROGRESS: Partial<Record<ServerImageTaskPhase, number>> = {
    queued: 12,
    generating: 42,
    "upstream-complete": 68,
    retrieving: 82,
    persisted: 94,
};

function PendingImageCard({ phase = "queued" }: { phase?: ServerImageTaskPhase }) {
    const label = IMAGE_PHASE_LABELS[phase] || "生成中";
    const progress = IMAGE_PHASE_PROGRESS[phase] || 24;
    return (
        <div className="relative aspect-square overflow-hidden rounded-lg border border-dashed border-stone-300 bg-stone-50 dark:border-stone-700 dark:bg-stone-900">
            ...
            />
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-8 text-sm text-stone-600 dark:text-stone-300" aria-live="polite">
                <LoaderCircle className="size-6" />
                <span>{label}</span>
                <div className="h-1 w-full max-w-40 overflow-hidden rounded-full bg-stone-200 dark:bg-stone-700">
                    <div className="h-full rounded-full bg-stone-700 transition-[width] duration-300 dark:bg-stone-200" style={{ width: `${progress}%` }} />
                </div>
            </div>
        </div>
    );
...
async function readStoredLogs(logStore: WorkbenchLogStore) {
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    const references = await Promise.all(
        (log.references || []).map(async (item) => ({
            ...item,
            dataUrl: item.storageKey ? await resolveImageUrl(item.storageKey, "") : item.dataUrl,
        })),
    );
    ...
    return {
        ...
        taskIds: Array.isArray(log.taskIds) ? log.taskIds.filter((taskId): taskId is string => typeof taskId === "string") : undefined,
        images,
