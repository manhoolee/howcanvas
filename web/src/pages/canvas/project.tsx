...
const TEXT_NODE_OUTPUT_SYSTEM_PROMPT = `你是文字卡片的正文润色器。无论用户输入中包含原文、修改要求、参考资料或其他上下文，你的输出都只能是最终可直接替换文字卡片正文的文本。

严格遵守：
1. 只输出最终正文，不输出分析、推理、解释、修改过程、前言或后记。
2. 不复述原文、用户要求、参考资料或任何上下文。
3. 不添加“润色后”“最终文本”“以下是”等提示语，不使用引号或代码围栏包裹正文。
4. 除非用户明确要求，否则不额外添加标题、标签、Markdown 标记或无关段落。
5. 保留用户要求的语言、段落结构和必要格式。`;
...
function InfiniteCanvasPage() {
    ...
    const resumedImageTasksRef = useRef(new Set<string>());
    const resumedVideoTasksRef = useRef(new Set<string>());
    ...
    const recoverVideoTask = useCallback(
        async (node: CanvasNodeData, suppliedTaskId?: string) => {
            const model = node.metadata?.videoTaskModel || node.metadata?.model || effectiveConfig.videoModel;
            const taskId = suppliedTaskId || node.metadata?.videoTaskId || (await askVideoTaskId());
            if (!taskId) return;
            const provider = node.metadata?.videoTaskProvider || (model.toLowerCase().includes("seedance") ? "seedance" : "openai");
            const task: VideoGenerationTask = { id: taskId, provider, model };
            const generationConfig = { ...buildGenerationConfig(effectiveConfig, node, "video"), model };
            ...
            try {
                ...
                message.success("视频结果已取回");
            } catch (error) {
                if (isGenerationCanceled(error)) return;
                const errorDetails = error instanceof Error ? error.message : "视频结果查询失败";
    ...
    const refreshNodeTaskStatus = useCallback(
        async (nodeId: string) => {
            ...
            if (!node) throw new Error("画布元素不存在");
            const checkedAt = new Date().toISOString();

            try {
                if (node.type === CanvasNodeType.Image && node.metadata?.serverTaskId) {
                    const state = await refreshImageGenerationTask(node.metadata.serverTaskId);
                    ...
                    const uploaded = await storeGeneratedImage(state.images[0]);
                        ...
                        prev.map((item) => {
                            ...
                            return {
                                ...
                                metadata: {
                                    ...
                                    serverTaskId: state.task.id,
                                    taskStatus: state.task.status,
                                    taskStatusUpdatedAt: state.task.updatedAt || checkedAt,
                                    errorDetails: undefined,
                                },
                            };
                        }),
                    );
                    acknowledgeImageTaskAfterRender(state.task.id);
                    message.success("任务已成功，图片已重新下载");
                    return;
                }
                ...
                if (node.type === CanvasNodeType.Video && node.metadata?.videoTaskId) {
                    const model = node.metadata.videoTaskModel || node.metadata.model || effectiveConfig.videoModel;
                    const provider = node.metadata.videoTaskProvider || (model.toLowerCase().includes("seedance") ? "seedance" : "openai");
                    if (provider === "plugin") throw new Error("插件视频任务无法在页面重载后查询");
                    const task: VideoGenerationTask = { id: node.metadata.videoTaskId, provider, model };
                    const generationConfig = { ...buildGenerationConfig(effectiveConfig, node, "video"), model };
                    if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                        openConfigDialog(true);
                        throw new Error("请先完成当前视频模型的渠道配置");
                    }
                        ...
                        prev.map((item) => {
                            ...
                            return {
                                ...
                                metadata: {
                                    ...
                                    videoTaskModel: task.model,
                                    taskStatus: "succeeded",
                                    taskStatusUpdatedAt: checkedAt,
                                    errorDetails: undefined,
                                },
                            };
                        }),
                    );
                    message.success("任务已成功，视频已重新下载");
                    return;
                }

                message.info("该元素没有可查询的任务 ID");
            } catch (error) {
                const errorDetails = error instanceof Error ? error.message : "刷新任务状态失败";
                message.error(errorDetails);
            }
        },
    ...
    useEffect(() => {
        if (!projectLoaded) return;
        const pending = nodesRef.current.filter((node) => node.type === CanvasNodeType.Video && node.metadata?.status === NODE_STATUS_LOADING && node.metadata.videoTaskId);
        pending.forEach((pendingNode) => {
            const taskId = pendingNode.metadata?.videoTaskId;
            if (!taskId || resumedVideoTasksRef.current.has(taskId)) return;
            resumedVideoTasksRef.current.add(taskId);
            void recoverVideoTask(pendingNode, taskId).finally(() => resumedVideoTasksRef.current.delete(taskId));
        });
    }, [projectLoaded, recoverVideoTask]);
    ...
    const createGroup = useCallback(() => {
        ...
        const selectedNodes = nodesRef.current.filter((node) => selectedIds.has(node.id) && node.type !== CanvasNodeType.Group);
        ...
        group.position = { x: bounds.left - sidePadding, y: bounds.top - topPadding };
        group.width = Math.max(360, bounds.right - bounds.left + sidePadding * 2);
        group.height = Math.max(260, bounds.bottom - bounds.top + topPadding + bottomPadding);

        setNodes((prev) => [group, ...prev.map((node) => (selectedIds.has(node.id) && node.type !== CanvasNodeType.Group ? { ...node, metadata: { ...node.metadata, groupId: group.id } } : node))]);
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`浩瀚画布 ${useCanvasStore.getState().projects.length + 1}`);
        navigate(`/canvas/${id}`);
    ...
    const exportCurrentProject = useCallback(async () => {
        ...
        try {
            await exportCanvasProjects([project], project.title || "浩瀚画布");
    ...
    const handleNodeMouseDown = useCallback((event: ReactMouseEvent, nodeId: string) => {
        ...
        let previousSize = -1;
        while (previousSize !== dragIds.size) {
            previousSize = dragIds.size;
            currentNodes.forEach((node) => {
                if (!dragIds.has(node.id)) return;
                node.metadata?.batchChildIds?.forEach((childId) => dragIds.add(childId));
                if (node.type === CanvasNodeType.Group) currentNodes.forEach((child) => child.metadata?.groupId === node.id && dragIds.add(child.id));
            });
        }
        dragRef.current = {
            isDraggingNode: true,
        ...
        const handlePaste = (event: ClipboardEvent) => {
            ...
            const itemFiles = Array.from(event.clipboardData?.items || [])
                ...
                .map((item, index) => {
                    ...
                    return blob ? new File([blob], blob.name || `clipboard-image-${index + 1}.png`, { type: blob.type || "image/png" }) : null;
                })
                .filter((file): file is File => Boolean(file));
            const imageFiles = itemFiles.length ? itemFiles : Array.from(event.clipboardData?.files || []).filter((file) => file.type.startsWith("image/"));

            if (imageFiles.length) {
                event.preventDefault();
                const center = getCanvasCenter();
                void Promise.all(imageFiles.map((file, index) => createImageFileNode(file, { x: center.x + index * 32, y: center.y + index * 32 })))
                    .then(() => message.success(imageFiles.length === 1 ? "已从剪贴板添加图片" : `已从剪贴板添加 ${imageFiles.length} 张图片`))
                    .catch((error) => {
                        console.error(error);
                        message.error("剪贴板图片添加失败");
                    });
                return;
            }
    ...
    }, [copySelectedNodes, deleteConnection, deleteNodes, redoCanvas, selectedConnectionId, setConnecting, undoCanvas]);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            setMaskEditNodeId(null);
            try {
                const uploaded = await uploadImage(payload.imageDataUrl);
                const childId = nanoid();
                ...
                setSelectedNodeIds(new Set([childId]));
                setSelectedConnectionId(null);
                message.success("已生成画笔修改后的新图片节点");
            } catch (error) {
                message.error(error instanceof Error ? error.message : "生成新图片失败");
            }
        },
        [message],
    );
    ...
    const generateAngleNode = useCallback(
        ...
        [effectiveConfig, finishGenerationRequest, imageTaskOptions, openConfigDialog, startGenerationRequest],
    );
    ...
    const handleGenerateNode = useCallback(
                                    ...
                                    return prev.map((node) => {
                                        ...
                                        if (node.id === rootId && (targetId === rootId || !root?.metadata?.primaryImageId))
                                            return {
                                                ...
                                                metadata: { ...node.metadata, ...imageMetadata(uploaded), primaryImageId: targetId, taskStatus: "succeeded", taskStatusUpdatedAt: new Date().toISOString() },
                                            };
                                        if (node.id === targetId)
                                            return {
                                                ...node,
                                                ...
                                                width: imageSize.width,
                                                height: imageSize.height,
                                                metadata: { ...node.metadata, ...imageMetadata(uploaded), taskStatus: "succeeded", taskStatusUpdatedAt: new Date().toISOString() },
                                            };
                                        return node;
                                    });
                                });
                ...
                if (mode === "video") {
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, videoTaskOptions(videoId, controller)),
                        );
                ...
                const answers = await Promise.all(
                    textTargetIds.map((targetNodeId) => {
                        ...
                        return requestImageQuestion(
                            generationConfig,
                            [{ role: "system", content: TEXT_NODE_OUTPUT_SYSTEM_PROMPT }, ...buildNodeResponseMessages({ ...generationContext, prompt: effectivePrompt })],
                            (text) => {
                                localStreamed = text;
                                streamed = text;
        ...
        [effectiveConfig, finishGenerationRequest, imageTaskOptions, isAiConfigReady, message, openConfigDialog, startGenerationRequest, videoTaskOptions],
    );
    useEffect(() => {
            ...
            try {
                if (node.type === CanvasNodeType.Text) {
                    ...
                    const answer = await requestImageQuestion(
                        generationConfig,
                        [{ role: "system", content: TEXT_NODE_OUTPUT_SYSTEM_PROMPT }, ...buildNodeResponseMessages({ ...context, prompt })],
                        (text) => {
                            streamed = text;
                ...
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], videoTaskOptions(node.id, controller)));
                    const videoSize = fitNodeSize(video.width || node.width, video.height || node.height, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
                    setNodes((prev) =>
                        prev.map((item) =>
                            item.id === node.id
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <CanvasSidePanel nodes={nodes} selectedNodeIds={selectedNodeIds} onFocusNode={focusNode} onRefreshNodeStatus={refreshNodeTaskStatus} onInsertAsset={handleAssetInsert} />
