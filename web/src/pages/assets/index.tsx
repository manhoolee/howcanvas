import { Copy, Download, FolderInput, Globe2, PencilLine, Search, Trash2, Upload } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { App, AutoComplete, Button, Card, Checkbox, Drawer, Empty, Form, Image, Input, Modal, Pagination, Select, Space, Tag, Typography } from "antd";
import { saveAs } from "file-saver";

import { useCopyText } from "@/hooks/use-copy-text";
import { formatBytes, readFileAsDataUrl } from "@/lib/image-utils";
import { uploadImage } from "@/services/image-storage";
import { publishAssetToPublic } from "@/services/public-assets";
...
export default function AssetsPage() {
    ...
    const [pendingFormValues, setPendingFormValues] = useState<Partial<AssetFormValues>>({});
    ...
    useEffect(() => {
        if (isAssetOpen) form.setFieldsValue(pendingFormValues);
    }, [form, isAssetOpen, pendingFormValues]);
    // 用户只能看见自己的资产（无归属的旧资产保持可见）
    const validAssets = useMemo(
        () => assets.filter((asset) => (asset.kind === "text" || asset.kind === "image" || asset.kind === "video") && (!asset.ownerId || asset.ownerId === currentUser?.id)),
        [assets, currentUser?.id],
    );

    // 已有文件夹列表（用于筛选与移动时的联想）
    const folders = useMemo(() => Array.from(new Set(validAssets.map((asset) => (asset.folder || "").trim()).filter(Boolean))).sort(), [validAssets]);
    ...
    const openCreate = () => {
        ...
        setImageDraft(null);
        setFormKind("text");
        setPendingFormValues({ kind: "text", title: "", coverUrl: "", tags: [], source: "手动添加", note: "", folder: folderFilter !== "all" && folderFilter !== "none" ? folderFilter : "", content: "" });
        setIsAssetOpen(true);
    };
    ...
    const publishOne = async (asset: Asset) => {
        try {
            await publishAssetToPublic(asset);
            message.success(`「${asset.title}」已设为公共资产`);
        } catch (error) {
    ...
    const moveSelected = () => {
        const targets = validAssets.filter((asset) => selectedIds.has(asset.id));
        if (!targets.length) {
            message.warning("请先勾选要移动的资产");
            return;
        }
        const folder = moveFolder.trim();
        targets.forEach((asset) => updateAsset(asset.id, { folder }));
        setMoveOpen(false);
        setMoveFolder("");
        setSelectMode(false);
        setSelectedIds(new Set());
        message.success(folder ? `已移动 ${targets.length} 个资产到「${folder}」` : `已将 ${targets.length} 个资产移出文件夹`);
    };

    const publishSelected = async () => {
        const targets = validAssets.filter((asset) => selectedIds.has(asset.id));
        if (!targets.length) {
            message.warning("请先勾选要设为公共的资产");
            return;
        }
        setPublishing(true);
        let ok = 0;
        ...
        setSelectMode(false);
        setSelectedIds(new Set());
        if (failed) message.warning(`已发布 ${ok} 个，失败 ${failed} 个`);
        else message.success(`已将 ${ok} 个资产设为公共资产`);
    };
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
                    ...
                    <div className="mx-auto mt-6 grid max-w-6xl gap-3 text-left">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="grid gap-2">
                                ...
                                <div className="grid gap-2 sm:grid-cols-[56px_minmax(0,1fr)] sm:items-center">
                                    <div className="text-xs font-medium text-stone-500 dark:text-stone-400">文件夹</div>
                                    <div className="flex flex-wrap gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                                {selectMode ? (
                                    <>
                                        <span className="text-sm text-stone-500 dark:text-stone-400">已选 {selectedIds.size} 项</span>
                                        <Button size="small" type="primary" loading={publishing} icon={<Globe2 className="size-3.5" />} onClick={() => void publishSelected()}>
                                            设为公共资产
                                        </Button>
                                        <Button size="small" icon={<FolderInput className="size-3.5" />} onClick={() => setMoveOpen(true)}>
                                            移动到文件夹
                                        </Button>
                                        <Button
                                            size="small"
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {visibleAssets.map((asset) => (
                            <AssetCard
                                ...
                                onPublish={() => void publishOne(asset)}
                    ...
                    <Form form={form} layout="vertical" requiredMark={false} initialValues={{ kind: "text", tags: [] }}>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <Form.Item name="folder" label="文件夹">
                                <AutoComplete options={folders.map((value) => ({ value }))} placeholder="留空为未分类，可输入新文件夹名" filterOption={(input, option) => String(option?.value ?? "").toLowerCase().includes(input.toLowerCase())} />
                            </Form.Item>
            ...
            <Modal title={`移动 ${selectedIds.size} 个资产到文件夹`} open={moveOpen} onCancel={() => setMoveOpen(false)} onOk={moveSelected} okText="移动" cancelText="取消">
...
function AssetCard({
    ...
    return (
        <Card
            ...
            cover={
                <button type="button" className="relative block w-full text-left" onClick={selectMode ? onToggleSelect : onOpen}>
                    {selectMode ? (
        ...
        >
                ...
                <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex shrink-0 flex-col items-end gap-1">
                            ...
                            {asset.folder ? <Tag className="m-0 max-w-24 truncate text-[11px]">📁 {asset.folder}</Tag> : null}
                        </div>
                    </div>
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pb-4">
                ...
                <Button size="small" icon={<Globe2 className="size-3.5" />} onClick={onPublish} title="设为公共资产">
                    公开
                </Button>
