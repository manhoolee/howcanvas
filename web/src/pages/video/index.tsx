...
import { getWorkbenchLogStore, syncWorkbenchLogs, type WorkbenchLogStore } from "@/services/workbench-cloud-sync";
...
export default function VideoPage() {
    ...
    const logStore = currentUserId ? getWorkbenchLogStore("video", currentUserId) : null;
    ...
    useEffect(() => {
        ...
        setStartedAt(0);
        setElapsedMs(0);
        if (currentUserId) void refreshLogs(currentUserId, getWorkbenchLogStore("video", currentUserId));
    ...
    const generate = async () => {
        ...
        try {
            ...
            void pollGenerationLog(log, snapshot.config, agentTaskId, ownerIdAtStart, authEpochAtStart);
        } catch (error) {
    ...
    const deleteSelectedLogs = () => {
        ...
        void Promise.all([deleteStoredMedia(mediaKeys), ...selectedLogIds.map((id) => store.removeItem(id))]).then(() => refreshLogs(ownerId, store));
    ...
    const saveLog = async (log: GenerationLog, resumePending = true) => {
        ...
        await syncWorkbenchLogs("video", ownerId, store, true);
        await refreshLogs(ownerId, store, resumePending);
    };

    const refreshLogs = async (ownerId: string, store: WorkbenchLogStore, resumePending = true) => {
        try {
            await syncWorkbenchLogs("video", ownerId, store);
        } catch (error) {
            console.warn("[video] 工作台云同步失败", error);
        }
        if (ownerId !== activeUserIdRef.current) return [];
        const nextLogs = await readStoredLogs(store);
        setLogs(nextLogs);
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig, agentTaskId?: string, ownerIdAtStart = currentUserId, authEpochAtStart = getAuthEpoch()) => {
        ...
        try {
            ...
            if (isCurrentGeneration() && !activeLogIdsRef.current.size) {
                setRunning(false);
                setStartedAt(0);
            }
        }
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    ...
    const minimaxH3 = isMiniMaxH3VideoConfig({ ...config, model });
    return {
        ...config,
        model,
        videoModel: model,
        size: seedance ? normalizeSeedanceRatio(config.size) : minimaxH3 ? normalizeMiniMaxH3Ratio(config.size) : normalizeVideoSize(config.size),
        videoSeconds: minimaxH3 ? String(normalizeMiniMaxH3Duration(config.videoSeconds)) : normalizeVideoSeconds(config.videoSeconds),
