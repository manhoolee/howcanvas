...
import { buildApiUrl, serverProxyHeaders, type AiConfig, type ModelCapability } from "@/stores/use-config-store";
...
const MAX_PLUGIN_SCRIPT_LENGTH = 100_000;
const FORBIDDEN_PLUGIN_TOKENS = /\b(?:window|document|globalThis|localStorage|sessionStorage|navigator|location|WebSocket|XMLHttpRequest|eval|Function|importScripts)\b/;

function validatePluginScript(script: string) {
    if (script.length > MAX_PLUGIN_SCRIPT_LENGTH) throw new Error(`模型插件脚本过长，最多允许 ${MAX_PLUGIN_SCRIPT_LENGTH} 个字符`);
    if (FORBIDDEN_PLUGIN_TOKENS.test(script)) throw new Error("模型插件脚本包含被禁止的浏览器或动态执行对象");
}
...
function pluginUrl(config: AiConfig, path: string) {
    if (/^https?:/i.test(path)) {
        const baseOrigin = new URL(config.baseUrl, typeof location !== "undefined" ? location.origin : "http://localhost").origin;
        const requested = new URL(path).origin;
        if (requested !== baseOrigin) throw new Error("模型插件只能请求当前渠道 Base URL 的 origin");
        return path;
    }
...
function createPluginHttp(config: AiConfig, options?: RequestOptions): PluginHttp {
    const run = async (method: "get" | "post", path: string, body: unknown, opts?: PluginHttpOptions) => {
        ...
        const response = await axios.request({
            ...
            params: opts?.params,
            headers: pluginHeaders({ Authorization: `Bearer ${config.apiKey}`, ...opts?.headers, ...serverProxyHeaders(config) }, method === "post" && !isForm && body !== undefined),
...
function createPluginRequest(config: AiConfig, options?: RequestOptions) {
    return async (requestConfig: AxiosRequestConfig & { url: string }) => {
        const response = await axios.request({ ...requestConfig, headers: { ...requestConfig.headers, ...serverProxyHeaders(config) }, url: pluginUrl(config, requestConfig.url), signal: options?.signal });
        return response.data;
    };
}
...
export async function runModelPlugin<T = unknown>(args: RunPluginArgs): Promise<T> {
    const { config } = args;
    validatePluginScript(args.script);
...
for (const dataUrl of images) {
  ...
  form.append("image", new Blob([bytes], { type: match[1] || "image/png" }), "ref.png");
}
