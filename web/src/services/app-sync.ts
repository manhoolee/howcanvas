...
import { getWorkbenchLogStore, type WorkbenchLogStore } from "@/services/workbench-cloud-sync";
...
export async function syncAppDataToWebdav(config: WebdavSyncConfig, onProgress?: AppSyncProgress): Promise<AppSyncResult> {
    ...
    if (!ownerId) throw new Error("请先登录再同步工作台数据");
    const imageLogStore = getWorkbenchLogStore("image", ownerId);
    const videoLogStore = getWorkbenchLogStore("video", ownerId);
...
async function readStoredLogs(store: WorkbenchLogStore) {
