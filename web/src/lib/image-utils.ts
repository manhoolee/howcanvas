...
export function dataUrlToBlob(dataUrl: string, fallbackMimeType = "application/octet-stream") {
    const commaIndex = dataUrl.indexOf(",");
    if (!dataUrl.startsWith("data:") || commaIndex < 0) throw new Error("图片 Data URL 格式无效");
    const metadata = dataUrl.slice(5, commaIndex);
    const payload = dataUrl.slice(commaIndex + 1);
    const isBase64 = /;base64$/i.test(metadata);
    const mimeType = metadata.split(";", 1)[0] || fallbackMimeType;
    const bytes = isBase64 ? Uint8Array.from(atob(payload), (character) => character.charCodeAt(0)) : new TextEncoder().encode(decodeURIComponent(payload));
...
export function dataUrlToFile(image: ReferenceImage) {
    ...
    return new File([blob], image.name || "reference.png", { type: blob.type });
}
