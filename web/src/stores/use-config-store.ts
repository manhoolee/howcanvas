    ...
    persist(
        ...
        }),
        {
            ...
            migrate: (persisted) => {
                ...
                // 旧版本曾将工作台默认张数保存为 3；只迁移这个旧默认值，
                // 不覆盖用户已经主动选择的其它张数。
                if (config?.count === "3") config.count = "1";
                // 画布图像节点也曾默认生成 3 张；同步迁移到单张。
                if (config.canvasImageCount === "3") config.canvasImageCount = "1";
                return { config, webdav: { ...defaultWebdavSyncConfig, ...(state.webdav || {}) } };
            },
            // 敏感信息（API Key、WebDAV 密码）不落盘：真实密钥只存在服务器 .env，
            // 「服务器渠道」的凭据会在每次登录时由 applyServerAiConfig 重新注入内存。
            partialize: (state) => ({
                config: {
                    ...state.config,
                    apiKey: "",
...
export function serverProxyHeaders(config: Pick<AiConfig, "baseUrl" | "model">): Record<string, string> {
    return config.baseUrl.trim().startsWith("/api/ai/") && config.model.trim()
        ? { "X-Infinite-Canvas-Model": config.model.trim() }
        : {};
}

export function isServerProxyModel(config: AiConfig, value: string) {
    return resolveModelChannel(config, value).baseUrl.trim().startsWith("/api/ai/");
}
