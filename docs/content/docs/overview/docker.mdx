---
title: Docker 部署
description: 使用 Docker Compose 部署 HowCanvas
---
...
当前部署由前端、后端和网关组成。登录后的画布 JSON 与画布媒体保存在后端挂载的数据目录 `server-data/`，按账户隔离；浏览器保留版本化缓存。第三方提示词由前端通过 CDN/备用地址拉取。
...
services:
  app:
    image: ghcr.io/manhoolee/infinite-canvas:latest
    ports:
      - "3000:3000"
    environment:
...
docker run -d -p 3000:3000 \
  ...
  ghcr.io/manhoolee/infinite-canvas:latest
```
