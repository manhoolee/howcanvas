---
title: Codex App 插件
description: 在 Codex App 中安装并使用 HowCanvas 插件
---

# Codex App 插件

HowCanvas 提供了一个 Codex App 插件，用来把 Codex 和本地 Canvas Agent 连接起来。安装后，你可以直接在 Codex 中说“打开 HowCanvas”，让 Codex 打开画布、读取节点、创建节点并触发生成流程。
...
```text
请从 https://github.com/manhoolee/howcanvas.git 安装 HowCanvas Codex 插件。
...
```bash
mkdir -p ~/plugins
git clone https://github.com/manhoolee/howcanvas.git ~/plugins/howcanvas
```

确保 `~/.agents/plugins/marketplace.json` 中有 HowCanvas 条目。这里的 `path` 要指向仓库里的插件子目录：
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "local",
        "path": "./plugins/howcanvas/plugins/infinite-canvas"
      },
      "policy": {
...
如果你正在 HowCanvas 仓库中开发插件，可以直接使用仓库自带的 marketplace：

```bash
cd /path/to/howcanvas
...
```text
打开 HowCanvas 新建画布并连接 Codex
```

插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开 `https://can.hoosland.com/` 并自动新建、连接画布。只有明确要求使用本地项目时，插件才会启动本地前端。
