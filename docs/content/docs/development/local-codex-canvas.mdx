...
当前账户的画布主数据保存在服务器，节点、连线、选区和视口在前端 React 状态中维护并使用浏览器缓存；Codex 仍只能通过 `Canvas Agent` 暴露的工具协议发出“读取画布”或“执行画布操作”的请求。画布写入后由账户同步层保存到服务器。
...
在画布中点选节点后，右侧对话输入区会显示当前选区的数量、节点类型图标和标题。发送消息时，选中节点的 ID、类型、标题、文本或提示词摘要及任务状态会作为本轮紧凑上下文随请求发送；本地 Agent 仍可通过 `canvas_get_selection` 读取最新完整选区。已发送的用户消息会保留发送瞬间的选区示意，取消选择或切换画布不会改写旧消息。

媒体节点只传递安全的元数据摘要，不会把 Blob、Data URL 或完整媒体内容复制进对话历史。
