...
仓库内提供了 Codex app 插件：`plugins/infinite-canvas`。在 Codex app 中添加本仓库的 marketplace 后，可以安装 `HowCanvas` 插件；插件会注册同一个 `infinite-canvas` MCP，并带上画布操作说明。
...
使用时可以直接在 Codex 里说“打开 HowCanvas”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后在右侧打开 `https://can.hoosland.com/` 并自动新建、连接画布；只有明确要求使用本地项目时才会启动本地前端。
