...
- 外部服务请求统一放在 `web/src/services/api/`；服务器管理的 AI 渠道必须经 `/api/ai/<channel-id>` 代理，不得把真实 API Key 下发到浏览器。
...
- 每个版本必须撰写独立的升级说明，至少包含升级前备份、配置变化、数据迁移、升级步骤、升级后验证、回滚方式和已知事项，并发布到 GitHub Release 或对应的发布页。
- 将当前未提交的代码全部提交到 Git。
- 提交完成后，给当前提交打最新版本号对应的 tag，例如 `v0.0.5`。
- 生产上线完成后，在发布说明或 PR 中记录上线日期、环境、备份位置、健康检查与回归结果，同时列出未解决风险。
- 发版本流程中不要执行编译、测试或构建，除非用户明确要求。

## 项目注意事项

- 延长媒体下载超时时，必须同步检查底层 HTTP 客户端的响应头与响应体等待限制，并验证响应体读取阶段的超时；不能只修改外层计时器。

- 账户画布、工作台记录和“我的素材”已同步到项目后端；浏览器仍保留版本化缓存与媒体 Blob 缓存，不要把缓存误当服务器主数据。
- 服务器管理的 AI API Key 保存在服务端数据目录的 `channels.json` 与受限的 `channel-credentials.json`，后者保留旧任务所需的历史凭据；两者一起保护和备份，密钥不进入台账或前端。用户自己的 WebDAV 凭据仍属浏览器本地配置。
- Docker 部署链路已验证前端、后端与网关三容器；公网上线仍必须单独验证 HTTPS、`COOKIE_SECURE`、持久化目录备份和健康检查。
- 生产服务器的登录、诊断和部署只使用普通 SSH（当前端口 `2222`）；不要改用网页控制台或浏览器登录。上线目标是主站画布 Agent，独立视觉工作台仅做路由保护和健康检查。
